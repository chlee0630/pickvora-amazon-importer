...
} from "./analytics/fulfillment-exception-analytics.service.js";
...
export async function getOperationsDashboard(shop, filters) {
  try {
    const [
      ...
      getFulfillmentExceptionSummary({ ...filters, shop }),
      getFulfillmentExceptionDetails({ ...filters, shop }, {
        page: filters.page,
...
async function formatManualReviewOrders(orders) {
  ...
  return orders.map((order) => {
    ...
    return {
      ...
      providerFailureCode: order.providerFailureCode,
      providerFailureMessage: redactDashboardText(order.providerFailureMessage),
      providerAttemptCount: order.providerAttemptCount || 0,
      providerLastAttemptAt: order.providerLastAttemptAt,
      failureReason: redactDashboardText(order.lastError || order.providerFailureMessage || queueJob?.failureReason || queueJob?.lastError),
