...
async function main() {
  ...
  try {
    ...
    const plannedConfig = buildDevFraudRefundE2EConfigPlan({ shop: targetShop });
    ...
    if (!applyRequested) {
      console.log(JSON.stringify({
        ...
        currentConfig,
        plannedConfig,
        applyCommand: "$env:TARGET_SHOP='pickvora-dev.myshopify.com'; $env:FRAUD_ORDER_CANCEL_ENABLED='true'; $env:FRAUD_ORDER_REFUND_ENABLED='true'; $env:CONFIRM_DEV_REFUND_E2E='true'; node scripts/enable-dev-fraud-refund-e2e-config.js --apply",
      }, null, 2));
      return;
    }

    const updatedConfig = await enableDevFraudRefundE2EConfig({
      shop: targetShop,
